...
export default function HRDashboard() {
  ...
  const quickActions = [
    { label: "Post New Job", icon: FileText, color: "bg-primary hover:bg-primary/90" },
    { label: "View Candidates", icon: Users, color: "bg-blue-600 hover:bg-blue-700" },
    { label: "Manage Referrals", icon: UserPlus, color: "bg-emerald-600 hover:bg-emerald-700" },
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="flex items-center justify-between mb-3">
                    <div className={`p-2 rounded-lg ${item.label === "Active Jobs" ? "bg-primary/10 text-primary" : "bg-black/5 dark:bg-white/10 text-muted-foreground"}`}>
                      <Icon size={20} />
                    </div>
                    <ArrowRight 
                      size={16} 
                  ...
                  {item.label === "Active Jobs" && activeJobs.length > 0 && (
                    <div className="mt-3">
                      <div className="h-1 w-full bg-primary/20 rounded-full overflow-hidden">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{ width: `${Math.min((activeJobs.length / 10) * 100, 100)}%` }}
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {totalCandidates} total candidates across {pipelineStages.length} stages
              </p>
            </div>
            ...
            {pipelineStages.map((stage, index) => {
              ...
              return (
                <motion.div
                  ...
                  whileHover={{ y: -2 }}
                  onClick={() => navigate(`/candidates?stage=${stage.toLowerCase()}`)}
                  className="text-center group/pipeline cursor-pointer"
                >
                  <div className="relative">
                    <div className="w-14 h-14 mx-auto mb-3 rounded-full bg-gradient-to-br from-primary/10 to-primary/5 flex items-center justify-center group-hover/pipeline:from-primary/20 group-hover/pipeline:to-primary/10 transition-all duration-300">
                  ...
                  <div className="flex items-center justify-center mt-2">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {stage === "Hired" ? "Final stage" : `${pipelineStages[index + 1] || "Next"} →`}
                    </span>
                  </div>
                </motion.div>
              );
